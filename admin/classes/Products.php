...
class Products
{
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Ecommerce-master/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Produk Berhasil Ditambahkan'];
					}else{
						return ['status'=> 303, 'message'=> 'Gagal'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Gagal Mengunggah Gambar'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Ukuran Gambar Maksimal 2MB'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Invalid Gambar Format [Valid Formats : jpg, jpeg, png]'];
		}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Ecommerce-master/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Produk Sukses Dirubah'];
					}else{
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Produk Berhasil di Update'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to run query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=> 'Invalid Produk id'];
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Produk Dihapus dari Sistem'];
			}else{
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Kategori Berhasil Di Update'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Bahan Berhasil Ditambahkan'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to run query'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Bahan Dihapus'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Brand Di Update'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
...
if (isset($_POST['add_product'])) {
	...
	&& !empty($_FILES['product_image']['name'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Silahkan Isi Form Dengan Lengkap']);
		exit();
	}
