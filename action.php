...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Bahan</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
				...
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$.$pro_price
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Pesan</button>
								</div>
							</div>
						</div>	
			";
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Produk Sudah Dimasukkan Dalam Keranjang</b>
				</div>
			";//not in video
		} else {
...
if (isset($_POST["Common"])) {
	...
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			?>
				<a style="width: 100%;" href="cart.php" class="btn btn-warning">Detail&nbsp;&nbsp;<span class="glyphicon glyphicon-edit"></span></a>
			<?php
			exit();
		}
	}
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right; margin-right: 80px;" name="login_user_with_product" class="btn btn-info btn-lg" value="Bayar Pesanan" >
							</form>';
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="ukirq@store.com">
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Produk di Hapus Dari Keranjang</b>
				</div>";
		exit();
	}
}
